...
int main() {
  ...
  const auto store_status = pp_key_vault_store(
      PP_KEY_VAULT_ABI_VERSION, bytes(record), {secret.data(), secret.size()});
  if (store_status != PP_KEY_VAULT_OK) {
    std::fprintf(stderr, "key vault store failed with status %d\n",
                 static_cast<int>(store_status));
  }
  assert(store_status == PP_KEY_VAULT_OK);
  size_t required = 0;
