...
CLIENT_APP_BUILD="$BUILD_ROOT/macos-client/ToyTalk.app"
...
swiftc -O \
  ...
  -o "$CLIENT_APP_BUILD/Contents/MacOS/ToyTalk"
sed "s/@VERSION@/$VERSION/g" packaging/macos/ClientInfo.plist.in > "$CLIENT_APP_BUILD/Contents/Info.plist"
...
if [ -n "${PLUSHPAL_CODESIGN_IDENTITY:-}" ]; then
  ...
  codesign --force --options runtime --timestamp --entitlements "$CLIENT_ENTITLEMENTS" \
    --sign "$PLUSHPAL_CODESIGN_IDENTITY" "$CLIENT_APP_BUILD"
  codesign --force --options runtime --timestamp --entitlements "$CLIENT_ENTITLEMENTS" \
  ...
  codesign --force --sign - "$CLIENT_APP_BUILD"
...
(cd "$OUTPUT" && COPYFILE_DISABLE=1 zip -X -q -y -r "ToyTalk-$VERSION-macos.zip" "ToyTalk Hub.app")
