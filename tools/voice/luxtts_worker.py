#!/usr/bin/env python3
"""Persistent LuxTTS worker for PlushPal Station.

The one-shot ``luxtts_tts.py`` wrapper is intentionally simple, but it pays the
Python startup, model load, and prompt-encoding costs on every synthesis.  This
worker keeps the LuxTTS model alive and caches encoded prompts by reference
hash, while preserving the exact generation settings used by the quality bakeoff.
"""

from __future__ import annotations

import argparse
import json
import random
import sys
import traceback
from pathlib import Path
from typing import Any


PROTOCOL_STDOUT = sys.stdout
sys.stdout = sys.stderr

ROOT = Path(__file__).resolve().parents[2]
LUX_ROOT_CANDIDATES = [
    ROOT / "third_party" / "LuxTTS",
    Path(__file__).resolve().parents[1] / "third_party" / "LuxTTS",
]
LUX_ROOT = next((path for path in LUX_ROOT_CANDIDATES if path.is_dir()), LUX_ROOT_CANDIDATES[0])


def _send(payload: dict[str, Any]) -> None:
    PROTOCOL_STDOUT.write(json.dumps(payload, separators=(",", ":")) + "\n")
    PROTOCOL_STDOUT.flush()


def _seed_everything(seed: int | None) -> None:
    if seed is None:
        return
    random.seed(seed)
    try:
        import numpy as np

        np.random.seed(seed)
    except Exception:
        pass
    try:
        import torch

        torch.manual_seed(seed)
        if torch.backends.mps.is_available():
            torch.mps.manual_seed(seed)
    except Exception:
        pass


def _patch_luxtts_prompt_processing() -> None:
    import librosa
    import torch
    import zipvoice.luxvoice as luxvoice
    from zipvoice.utils.infer import rms_norm

    @torch.inference_mode()
    def process_audio_with_long_prompt_support(
        audio,
        transcriber,
        tokenizer,
        feature_extractor,
        device,
        target_rms=0.1,
        duration=4,
        feat_scale=0.1,
    ):
        prompt_wav, _ = librosa.load(audio, sr=24000, duration=duration)
        prompt_wav2, _ = librosa.load(audio, sr=16000, duration=duration)
        transcription_kwargs = {"return_timestamps": True} if duration and duration > 30 else {}
        prompt_text = transcriber(prompt_wav2, **transcription_kwargs)["text"]
        print(prompt_text, file=sys.stderr)

        prompt_wav_tensor = torch.from_numpy(prompt_wav).unsqueeze(0)
        prompt_wav_tensor, prompt_rms = rms_norm(prompt_wav_tensor, target_rms)

        prompt_features = feature_extractor.extract(
            prompt_wav_tensor, sampling_rate=24000
        ).to(device)
        prompt_features = prompt_features.unsqueeze(0) * feat_scale
        prompt_features_lens = torch.tensor([prompt_features.size(1)], device=device)
        prompt_tokens = tokenizer.texts_to_token_ids([prompt_text])
        return prompt_tokens, prompt_features_lens, prompt_features, prompt_rms

    luxvoice.process_audio = process_audio_with_long_prompt_support


def main() -> int:
    parser = argparse.ArgumentParser(description="Run persistent LuxTTS worker")
    parser.add_argument("--model", default="YatharthS/LuxTTS")
    parser.add_argument("--device", default="mps")
    parser.add_argument("--threads", type=int, default=4)
    parser.add_argument("--ref-duration", type=float, default=180.0)
    parser.add_argument("--rms", type=float, default=0.01)
    parser.add_argument("--num-steps", type=int, default=8)
    parser.add_argument("--t-shift", type=float, default=0.9)
    parser.add_argument("--speed", type=float, default=0.88)
    parser.add_argument("--seed", type=int)
    parser.add_argument("--return-smooth", action="store_true")
    args = parser.parse_args()

    try:
        _seed_everything(args.seed)
        sys.path.insert(0, str(LUX_ROOT))

        import soundfile as sf
        import torch
        from huggingface_hub import snapshot_download
        from zipvoice.luxvoice import LuxTTS

        _patch_luxtts_prompt_processing()
        snapshot_download(args.model)
        snapshot_download("openai/whisper-base")
        model = LuxTTS(args.model, device=args.device, threads=args.threads)
        prompt_cache: dict[str, Any] = {}
        _send({"ok": True, "event": "ready"})

        for line in sys.stdin:
            line = line.strip()
            if not line:
                continue
            try:
                request = json.loads(line)
                command = request.get("command")
                if command == "shutdown":
                    _send({"ok": True, "event": "shutdown"})
                    return 0
                if command != "synthesize":
                    raise ValueError(f"unsupported command: {command}")

                reference = Path(str(request["reference"])).resolve()
                output = Path(str(request["output"])).resolve()
                text = str(request["text"]).strip()
                cache_key = str(request.get("cache_key") or reference)
                if not reference.is_file():
                    raise FileNotFoundError(reference)
                if not text:
                    raise ValueError("text is required")
                output.parent.mkdir(parents=True, exist_ok=True)

                encoded_prompt = prompt_cache.get(cache_key)
                cache_hit = encoded_prompt is not None
                if encoded_prompt is None:
                    encoded_prompt = model.encode_prompt(
                        str(reference),
                        duration=args.ref_duration,
                        rms=args.rms,
                    )
                    prompt_cache[cache_key] = encoded_prompt

                with torch.inference_mode():
                    final_wav = model.generate_speech(
                        text,
                        encoded_prompt,
                        num_steps=args.num_steps,
                        t_shift=args.t_shift,
                        speed=args.speed,
                        return_smooth=args.return_smooth,
                    )
                final_wav = final_wav.detach().cpu().numpy().squeeze()
                sf.write(str(output), final_wav, 48_000)
                _send({"ok": True, "cache_hit": cache_hit})
            except Exception as exc:  # pragma: no cover - process boundary
                traceback.print_exc(file=sys.stderr)
                _send({"ok": False, "error": str(exc)})
    except Exception as exc:  # pragma: no cover - process boundary
        traceback.print_exc(file=sys.stderr)
        _send({"ok": False, "event": "startup_failed", "error": str(exc)})
        return 2
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
