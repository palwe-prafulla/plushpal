...
| sentence-transformers/all-MiniLM-L6-v2 | Downloaded by ToyTalk Hub setup for the local current-info routing classifier; model card: <https://huggingface.co/sentence-transformers/all-MiniLM-L6-v2> | Hugging Face model card lists Apache-2.0; retain notices/model-card attribution and include dependencies such as sentence-transformers, transformers, scikit-learn, numpy, and torch in generated SBOM |
| Brave Search API | Optional developer/power-user API key for advanced Hub-owned current-info evidence experiments in Local AI mode | Review Brave Search API terms, attribution, quota, logging, and privacy obligations before distribution; no API key is bundled and it is not part of the normal parent setup UX |
