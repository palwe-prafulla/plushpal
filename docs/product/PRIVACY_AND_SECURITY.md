...
| Local debug logs expose family conversation content | Logs stay local on the Hub Mac; response-content logging is bounded and tagged for troubleshooting |
...
Hub logs may contain AI responses generated by Local AI, Gemini, or OpenAI in
`ToyTalk ai_response` lines. This is intentional local debugging data so a
technical user can see what model/provider actually answered. These response
logs are bounded, single-line escaped, and provider/model tagged, but they are
plain local log files rather than SQLCipher-encrypted database records. Treat
logs as private before sharing them.

Prefer structured status/error codes for prompts, credentials, paths, setup
state, and secrets. Do not log raw API keys, raw audio, SQLCipher keys, session
tokens, or full provider credentials.
